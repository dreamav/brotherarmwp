...
; (function () {
	...
	********************************************/
		...
		o.animate({ opacity: 1 }, 300, function () {
			...
			if (owlEle.find(".owl-stage").length === 0) {
				owlEle.owlCarousel({
					...
					autoHeightClass: config.autoHeightClass,
	...
	rgen.global_validation = {
		...
		msgpos: 'normal',
		msg: {
			email: { email: "Enter correct e-mail" }
		},
		subscribe_successMsg: "Ваши данные отправлены. Скоро мы с вами свяжемся",
		form_successMsg: "We will contact you soon!",
	...
	rgen.resetForm = function (form) {
		'use strict';
		$(form).find('input[type="text"], input[type="email"], input[type="phone"], textarea').val(null);
	}
			...
			$.ajax({
				...
				success: function (data) {
					if (data.status == 'error') {
						...
						if ( data.data.ya_goal != undefined ) {
							yaCounter44882842.reachGoal(data.data.ya_goal);
						}
						if ( data.data.ya_goal_fsent != undefined ) {
							yaCounter44882842.reachGoal(data.data.ya_goal_fsent);
						}
						
						if ( data.data.ga_goal != undefined ) {
							ga('send', 'event', 'Form', data.data.ga_goal);
						}
						if ( data.data.ga_goal_fsent != undefined ) {
							ga('send', 'event', 'Form', data.data.ga_goal_fsent);
						}


						if ( data.data.form_type == 26 ) {
							var handler = StripeCheckout.configure({
								key: 'pk_test_e1LuHg5zkqQJYv5ZEi9dCDpS',
								image: data.data.stripe_logo_sa,
								locale: 'auto',
								...
								token: function(token) {
									...
									$.ajax({
										url: data.data.stripe_payment,
										type: 'POST',
										dataType: "json",
										data: {token:token.id,sum:data.data.stripe_sum},
									})
									.success(function(data) {
										rgen.resetForm($form);
										$btn.button('reset');
										swal("Success!", data.data.success, "success");
							...
							handler.open({
								...
								description: data.data.quest_name,
								email: data.data.user_email,
								amount: data.data.stripe_sum
							});
							...
							if ( data.data.redirect != undefined ) {
								swal({
									type: "success",
									title: "Сейчас вы будете перемещены на страницу оплаты",
								...
								swal({
									type: "success",
									title: data.data.form_button_text,
									text: validate_data.successMsg,
									timer: timer
								}, function () {
									if ($form.attr('data-success-redirect') === 'y') {
	...
	jQuery(document).ready(function ($) {
		...
		if ($o.r) {
			...
			********************************************/
			if ($o.setpop) {
				for (var i = 0; i < $o.setpop.length; i++) {
					...
					/*console.log($o.setpop[i]);
					console.log($($o.setpop[i]).data('price'));*/
					// console.log($(pop).find('.pop-header').find('h2').find('span'));
					...
					$($o.setpop[i]).magnificPopup({
						...
						callbacks: {
							beforeOpen: function () {
								var clicked = $($(this)[0].items[0]),
									popupBlock = $(clicked.attr('href'));

								if (clicked.data('title') != undefined) {
									$(popupBlock).find('.pop-header').find('h2').text(clicked.data('title'));
									// $(popupBlock).find('.pop-body').find('img').attr('src', clicked.data('img'));
									$(popupBlock).find('.pop-body').find('.pop-quest-img').css('background-image', 'url('+clicked.data('img')+')');

									if ( clicked.data('desc')!= undefined ) {
										$(popupBlock).find('.pop-body').find('.quest-description').html(clicked.data('desc'));
									}
									if ( clicked.data('opisanie')!= undefined ) {
										$(popupBlock).find('.pop-body').find('.program-opisanie').html(clicked.data('opisanie'));
										$(popupBlock).find('.pop-body').find('.program-mechanika').html(clicked.data('mechanika'));
										$(popupBlock).find('.pop-body').find('.program-final').html(clicked.data('final'));
									}

									$(popupBlock).find('.pop-body').find('input[name=questName]').val(clicked.data('title'));
								}

								if ( clicked.data('p_count') != undefined ) {
									$(popupBlock).find('.pop-body').find('.pop-people-quantity').html(clicked.data('p_count'));
									$(popupBlock).find('.pop-body').find('input[name="people-quantity"]').val(clicked.data('p_count'));

									$(popupBlock).find('.pop-body').find('.pop-age-quantity').html(clicked.data('a_count'));
									$(popupBlock).find('.pop-body').find('input[name="age-quantity"]').val(clicked.data('a_count'));

									$(popupBlock).find('.pop-body').find('.pop-place').html(clicked.data('place'));
									$(popupBlock).find('.pop-body').find('input[name="place"]').val(clicked.data('place'));


									if ( clicked.data('formatmer_akt') != undefined || clicked.data('formatmer_int') != undefined || clicked.data('formatmer_creat') != undefined ) {
										var formatmer_text = "";
										if (clicked.data('formatmer_akt') != '') {formatmer_text = formatmer_text+clicked.data('formatmer_akt')+',';}
										if (clicked.data('formatmer_int') != '') {formatmer_text = formatmer_text+clicked.data('formatmer_int')+',';}
										if (clicked.data('formatmer_creat') != '') {formatmer_text = formatmer_text+clicked.data('formatmer_creat')+',';}

										formatmer_text = formatmer_text.substring(0, formatmer_text.length - 1);

										$(popupBlock).find('.pop-body').find('.pop-formatmer').html(formatmer_text);
										$(popupBlock).find('.pop-body').find('input[name="formatmer"]').val(formatmer_text);

									}
								}

								if ( clicked.data('shk_tarif') != undefined ) {
									$(popupBlock).find('.pop-body').find('input[name="shk_tarif"]').val(clicked.data('shk_tarif'));
								}

								if ( clicked.data('price') != undefined ) {
									$(popupBlock).find('.pop-header').find('h2').find('span').text(clicked.data('price'));
									$(popupBlock).find('.pop-body').find('input[name=price]').val(clicked.data('price'));
								}
			...
			********************************************/
			if ($o.videobg) {
				var mbyt_script_arr = ["/ru/lib/jquery.mb.YTPlayer/jquery.mb.YTPlayer.min.js"];
